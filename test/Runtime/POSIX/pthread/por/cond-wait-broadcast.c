...
// RUN: %klee --output-dir=%t.klee-out --posix-runtime --exit-on-error --no-schedule-forks --thread-scheduling=first --log-por-events %t.bc 2>&1 | FileCheck %s
...
// CHECK-NEXT: POR event: broadcast with current thread [[SEC_TID]] on cond. var [[COND]] and broadcasted threads: [[M_TID]]

...
// RUN: %klee --output-dir=%t.klee-out --exit-on-error --allow-external-sym-calls -detect-infinite-loops=0 --switch-type=internal %t.bc
// RUN: not test -f %t.klee-out/test000010.ktest

// RUN: rm -rf %t.klee-out
// RUN: %klee --output-dir=%t.klee-out --exit-on-error --allow-external-sym-calls -detect-infinite-loops=0 --switch-type=simple %t.bc

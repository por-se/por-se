...
int close(int fd) {
  ...
  if (f->socket) {
    r = kpr_close_socket(f);
  } else if (f->pipe) {
    if ((f->flags & eWriteable) != 0) {
      assert(f->pipe->writeFd == fd);
      f->pipe->writeFd = 0;
    } else if ((f->flags & eReadable) != 0) {
      assert(f->pipe->readFd == fd);
      f->pipe->readFd = 0;
    }
...
int fstatat(int fd, const char *path, struct stat *buf, int flags) {
  ...
  ret = syscall(__NR_newfstatat, (long)fd,
                 (path ? __concretize_string(path) : NULL), buf, (long)flags);
#else
  ret = syscall(__NR_fstatat64, (long)fd,
                 (path ? __concretize_string(path) : NULL), buf, (long)flags);
#endif
...
int select(int nfds, fd_set *read, fd_set *write,
  ...
  // TODO: just use the new poll api for compatibility

...
namespace por::event {
	...
	protected:
		lock_destroy(thread_id_t tid, std::shared_ptr<event> const& thread_predecessor, std::shared_ptr<event> const& lock_predecessor)
		: event(event_kind::lock_destroy, tid)
		, _predecessors{thread_predecessor, lock_predecessor}
		{
			...
			assert(
				lock_dependency()->kind() == event_kind::lock_create
				|| lock_dependency()->kind() == event_kind::lock_release
				|| (lock_dependency()->kind() == event_kind::lock_acquire && lock_dependency()->tid() == this->tid())
			);
		}

	public:
		static std::shared_ptr<lock_destroy> alloc(thread_id_t tid, std::shared_ptr<event> const& thread_predecessor, std::shared_ptr<event> const& lock_predecessor) {
			return std::make_shared<lock_destroy>(lock_destroy{tid, thread_predecessor, lock_predecessor});
		}
